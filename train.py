...
# Define the path to your dataset directory containing facades
...
# Custom dataset class to load facades without classes
class FacadeDataset(Dataset):
    ...
    def __getitem__(self, idx):
        ...
        return image, torch.tensor(0)  # Assign dummy label 0 to all images

# Create an instance of the FacadeDataset
...
def main():
    ...
    try:  # for unexpected error logging
        ...
        transform = transforms.Compose([
                        transforms.Resize((256, 256)),  # Resize the images to 64x64
                        transforms.ToTensor(),  # Convert images to tensors
                        transforms.Normalize((0.5, 0.5, 0.5), (0.5, 0.5, 0.5)),  # Normalize the image tensors
                    ])

# Load the CelebA dataset
#         train_dataset = CelebA(root=config['train_data_path'],
#                                 split='train',  # 'train', 'valid', or 'test'
#                                 target_type='attr',
#                                 transform=transform,
#                                 download=False) # Set to True if the dataset is not already downloaded
#         train_dataset = FacadeDataset(data_dir=config['train_data_path'], transform=transform)
